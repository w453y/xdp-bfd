...
**Kernel-side detection sweep** runs one global `bpf_timer` at 5ms, comparing `now - last_seen` against each session's negotiated detect time and pushing transitions to userspace over a ring buffer. XDP cannot see silence any more than it can originate — a dead link delivers no packets, so the program that would notice never runs. Detection therefore has to live outside the packet path, and it works even when userspace is fully starved.
...
bfdd is tighter at idle, and that is a real tradeoff rather than a rounding artifact: its detection runs on an hrtimer at almost exactly 3x the interval, while this engine's sweep runs on a 5ms tick, so detection quantizes to the next sweep — 0-5ms of granularity, about 1.3ms of mean overshoot. Under stress the figure is 33-34ms. That 1.3ms at idle buys scheduler immunity under load, which is the whole trade.
...
**RX-clocked TX requires an asynchronously-clocked peer.** Two RX-clocked implementations facing each other would echo into silence, because nobody sends first after a gap. The userspace slow-rate path doubles as the recovery spark and detection never depended on receiving, but the assumption should be written on the box. Two RX-clocked ends remain untested.
...
**Detection is quantized by the sweep.** 0-5ms of granularity above the RFC floor, about 1.3ms mean. Deliberate, and the cost of putting detection outside the packet path.
