		       ...
		       unsigned long long silent_ns, unsigned int alive_in, unsigned int want_alive)
{
	...
	if ((unsigned int)after.alive != want_alive) {
		printf("     alive is %u, want %u\n", (unsigned int)after.alive, want_alive);
		printf("FAIL %-40s\n", name);
		fails++;
	} else {
		printf("ok   %-40s alive %u\n", name, (unsigned int)after.alive);
	}
			       ...
			       unsigned int want_echo_alive)
{
	...
	if (after.echo_alive != want_echo_alive) {
		printf("     echo_alive is %u, want %u\n", after.echo_alive, want_echo_alive);
		bad = 1;
	}
	if ((unsigned int)after.alive != 1) {
...
#define MS BFD_AUTH_METICULOUS_SHA1
	...
	case_auth_reject("auth-window-wraps-cleanly", KS, "topsecret", 7, 0x00000002, 0,
			 0xFFFFFFFF, 1);
