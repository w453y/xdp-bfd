...
static void one_session(FILE *f, const struct session *s, int first)
{
	...
	fprintf(f, "%s\n    {\"lid\": %u, \"peer\": \"%s\", \"local\": \"%s\",", first ? "" : ",",
		s->lid, peer, local);
	fprintf(f, " \"family\": %d, \"multihop\": %s, \"min_ttl\": %u,",
		s->family == AF_INET6 ? 6 : 4, s->is_mhop ? "true" : "false", s->min_ttl);
	fprintf(f, " \"state\": \"%s\", \"diag\": %d,", bfd_state_str(s->state), s->diag);
	fprintf(f, " \"my_disc\": %u, \"remote_disc\": %u,", s->wire_disc, s->rdisc);
	fprintf(f, " \"up_events\": %u, \"down_events\": %u,", s->up_events, s->down_events);
	...
	fprintf(f, " \"last_detect_us\": %u, \"last_overshoot_us\": %u,", s->last_detect_us,
		s->last_overshoot_us);
	fprintf(f, " \"min_tx_us\": %u, \"min_rx_us\": %u, \"detect_mult\": %u,", s->min_tx_us,
		s->min_rx_us, s->detect_mult);
	fprintf(f, " \"remote_min_tx_us\": %u, \"remote_min_rx_us\": %u,", s->r_min_tx,
		s->r_min_rx);
	fprintf(f, " \"remote_detect_mult\": %u, \"remote_flags\": %u,", s->r_mult, s->r_flags);
	fprintf(f, " \"detect_basis_us\": %u, \"polling\": %s,", s->detect_iv_us,
	...
	fprintf(f, " \"kernel_detects\": %u, \"last_detect_lag_us\": %u,", s->kernel_detects,
		s->last_detect_lag_us);
	...
	fprintf(f,
		...
		!s->echo_rx_pkts ? "null" : (s->echo_alive_k ? "true" : "false"));
}
...
#undef BFD_STAT_NAME
	...
	fprintf(f, "  \"loop_passes\": %llu, \"loop_rx_wakeups\": %llu,\n",
		(unsigned long long)loop_passes, (unsigned long long)loop_rx_wakeups);
	fprintf(f, "  \"loop_gap_us\": [");
	for (int i = 0; i < 24; i++)
		fprintf(f, "%s%llu", i ? ", " : "", (unsigned long long)loop_gap_us[i]);
	fprintf(f, "],\n");

	fprintf(f, "  \"stats\": {");
	for (__u32 k = 0; k < BFD_STAT_MAX; k++)
		fprintf(f, "%s\"%s\": %llu", k ? ", " : "", names[k], slot_total(k, ncpu));
	fprintf(f, "},\n");
