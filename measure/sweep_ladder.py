...
# Where the lab is. These are the same variables tests/lib/lab.py defines
# on the code branch, with the same defaults, kept here because these
# instruments are checked out on their own. Override any of them in the
# environment rather than editing this.
#
...
#
# The distro vtysh is a trap worth naming: it exists on this box, it
# answers, and its daemons are not the ones running, so a rig that picks it
# up reports an empty mesh rather than an error.
PEER_HOST = os.environ.get("BFD_PEER_HOST", "w453y@10.66.0.2")
IFACE = os.environ.get("BFD_IFACE", "ens19")
ENGINE = os.environ.get("BFD_ENGINE",
                        os.path.expanduser("~/xdp-bfd/bfd_tx"))
