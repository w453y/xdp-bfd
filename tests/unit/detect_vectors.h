...
/* detect_vectors.h - the poll-aware detect rule, which fsm.c and bfd_xdp.c must
 * agree on. Their candidates differ: fsm.c takes max(r_min_tx, min_rx_us),
 * bfd_xdp.c clamps min_tx to its floor. Exact-gap vectors are engine-only, as
 * the XDP driver's gap carries clock skew.
 */
...
/* No prior interval, so step 0 takes the candidate. */
/* clang-format off */
static const struct dv_case dv_cases[] = {
	...
	/* A larger interval is a larger budget: at once. */
	{ "increase-applies-immediately", 10000, 0, 2, {
		{ 10000,     0, 10000 },
		{ 50000, 10000, 50000 },
	} },

	/* The 5ms advertisement still arrives 50ms after the last packet, so it waits. */
	...
	/* Our min_rx floors the candidate. */
	...
	/* The engine compares with <=. Engine-only. */
