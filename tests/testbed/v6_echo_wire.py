...
def check_fresh():
    ...
    if exe.endswith("(deleted)"):
        sys.exit(
            "running bfd_tx is a DELETED binary: rebuilt but not "
    ...
    if started < built:
        sys.exit(
            "running bfd_tx started %.0fs before %s was built: restart it"
...
def arm(name, local, peer, fwd, outdir):
    ...
    print(
        "    sent    (self-addressed, hlim 255): %d, [udp sum ok]: %d"
        % (len(tx), sumok)
    )
    print(
        "    returned(self-addressed, hlim 254): %d, payload matches a sent frame: %d"
        % (len(rx), matched)
    )
    print("    other v6 3785 frames on this address: %d" % other)
    print(
        "    engine: active=%s tx %s->%s rx %s->%s rtt_last=%sus alive=%s"
        % (
            e.get("active"),
            b.get("tx"),
            e.get("tx"),
...
def main():
    ...
    line = sh(
        "sudo %s -c 'show running-config' | grep -E '^ peer fd66::10:' | head -1"
        % args.vtysh
    )
    if not line.strip():
    ...
    vtysh_config(
        [line.rstrip(), "  echo-mode", "  echo transmit-interval %d" % args.interval_ms]
    )
    time.sleep(2)
    ...
    if not armed or not armed["echo"]["active"]:
        ...
        sys.exit(
            "echo did not arm (on=%s active=%s): nothing would transmit "
            "and every wire count below would be vacuous"
            % (armed and armed["echo"]["on"], armed and armed["echo"]["active"])
        )
    ...
    try:
        ...
        vtysh_config([line.rstrip(), "  no echo-mode"])
        rsh(args.peer_host, "sudo sysctl -qw net.ipv6.conf.all.forwarding=%s" % orig)
    ...
    for name, r in (("noforward", a), ("forward", b)):
        if r["sumok"] != r["tx"]:
            print(
                "FAIL %s: %d of %d outbound frames failed the checksum check"
                % (name, r["tx"] - r["sumok"], r["tx"])
            )
            ok = False
        else:
            print(
                "ok   %s: %d frames sent, all with a valid v6 UDP checksum"
                % (name, r["tx"])
            )
    if a["rx"] != 0:
        print(
            "FAIL negative arm: %d returns with the neighbour not forwarding" % a["rx"]
        )
        ok = False
    else:
    ...
    if b["rx"] == 0 or b["matched"] != b["rx"]:
        print(
            "FAIL forward arm: %d returns, %d matching a sent payload"
            % (b["rx"], b["matched"])
        )
        ok = False
    else:
        print(
            "ok   forward arm: %d returns, every payload matching one we sent" % b["rx"]
        )
    e = b["echo"]
    if not e.get("rx") or not e.get("rtt_last_us"):
        print(
            "FAIL engine did not demux the returns: rx=%s rtt_last_us=%s"
            % (e.get("rx"), e.get("rtt_last_us"))
        )
        ok = False
    else:
        print(
            "ok   engine demuxed: rx=%s rtt_last=%sus alive=%s"
