...
![Gap survival function by backend](benchmarks/charts/gap-survival.png)
...
m5-hardening branch (see milestones/04-hardening and benchmarks):
...
### After m5: a review-and-hardening pass (milestones/05-abi-refactor)
...
are in milestones/05-abi-refactor.

### After the review pass: IPv6 (milestones/07-ipv6)
...
### After IPv6: echo mode (milestones/08-echo)
...
The originator stayed v4 only after that, and the reason turned out to be an assumption rather than a measurement. FRR sources its IPv6 echoes at the peer rather than at itself, from which I inferred that a neighbour's forwarding plane would not loop a self-addressed v6 packet the way it loops a v4 one, and left it there. It loops. Ten injected, ten returned at hop limit 254 with the neighbour forwarding, none with it off, against a v4 control arm in the same rig — a ten-line injector, under a minute, and an inference that had been load-bearing for the whole milestone. FRR's choice was FRR's, not the kernel's. What followed was mostly the same GTSM trap sprung a second time: the v6 parser branch dropped every return before the echo path saw it, and it read the UDP header only after the hop-limit check, so the exception could not be written until the pointer moved. The v6 builder itself is unremarkable — 86 bytes, no IP checksum, a UDP checksum folded over a 40-byte pseudo-header instead of 12 — and the wire says it is right: 295 sent and 295 returned, every returned payload byte-identical to one that was sent, none returned at all with the neighbour's forwarding off. A second family gate was hiding in the stats dump, reporting v6 echo as inactive while it transmitted, and it was found only because the test needed that field as an oracle. That is the failure mode worth naming: a gate written during a single-family phase does not break anything when the phase ends, it just quietly reports the wrong thing until something reads it.
...
### After echo: multihop (milestones/09-multihop)
...
### After multihop: authentication (milestones/10-auth)

Every milestone before this one moved work into the driver because the work was cheap — compare a TTL, compare a discriminator, rewrite a header. Authentication asks for a keyed digest over the whole packet, and the kernel offers an XDP program nothing to compute one with: bpf_crypto_* is skcipher only, encrypt and decrypt, with no hashing and no HMAC anywhere in the BTF. The alternative was to leave authenticated sessions in userspace, which withdraws the engine's entire property from exactly the sessions an operator has asked to protect. So the digest is carried in the tree, in one header both planes include, because two implementations of one digest would disagree silently — every packet failing to authenticate on both sides, with nothing in either log to say which end was wrong. Writing HMAC-SHA1 that the verifier accepts is mostly a lesson in what the BPF target does to code that looks fine in C: the block compression and the padding step have to stay calls, because forced inline they expand six times between them and each expansion keeps its own message schedule, which overruns the 512-byte stack; the byte loops must not be unrolled, because sixty-four selects over a key pointer spill hard enough to need 800 bytes of frame; the API takes pre-padded blocks rather than a pointer and a length, because a copy loop bounded by a runtime length makes the verifier fork its state on every iteration; and both callees return a value laundered through an empty asm, because a bpf-to-bpf callee must leave a scalar in R0 and LLVM will happily propagate a constant return without ever emitting it. Ten vectors, four of them RFC 2202's own, checked twice — on the host and through the kernel by an entry point in the test object, because the BPF build is a different compilation answering to a different authority and agreeing with the host is not something to assume.

The ordering inside the fast path had to change. RFC 5880 discards an authenticated packet on a session with no key, and equally discards a bare packet on a session that has one — and the second half is the one that matters, because without it a peer strips authentication simply by not offering it and the session stays up looking perfectly healthy. That makes acceptance a property of the session rather than of the packet alone, so the program now looks the session up before it validates the header, keying on addresses the parser has already read so that nothing in the BFD header is trusted to perform the lookup. Getting under the verifier's 512-byte stack budget, which is charged against a whole call chain rather than a frame, took four separate reductions: the digest's scratch to a per-CPU map, the HMAC's two block buffers into one, its inner digest into the caller's output buffer, and sha1_finish back to inlined once it no longer owned a buffer. 656 bytes down to under 512, at 262915 instructions and 3153 states.

Keyed SHA1 went first, which is the opposite of what it looks like from outside. A keyed-SHA1 packet is always 52 bytes, so every copy has a constant bound; a simple-password packet is 24 plus 3 plus however long the key is, and a runtime length is what the verifier is worst at. Three attempts taught three things. A guarded loop over packet bytes does not survive the optimiser — it unrolls, folds the per-iteration bound check away with the index, and leaves the verifier looking at a constant offset past the range it has proven. Computing the payload offset by subtracting packet pointers is refused outright. And a bound of "not zero" is not a bound: the verifier still believed the length could be zero at the helper call and refused a zero-sized read. What works is bpf_xdp_load_bytes and bpf_xdp_store_bytes, which take the length as an argument and do the bounds check in the kernel, so one helper call carries the whole variable-length problem and everything downstream works on a fixed-size block.

That block also fixed a bug that had nothing to do with authentication and presented entirely as one. On IPv6 the session came up in userspace, handed over to the fast path, went silent, timed out, and repeated — 58 flaps in 30 seconds with both authentication counters flat. The payload contribution to the IPv6 UDP checksum was being assembled byte by byte into big-endian words while every other term in the fold was read straight out of memory, and a ones-complement sum is only byte-order agnostic when every term is accumulated the same way. Invisible on IPv4, which sends no UDP checksum at all; fatal on IPv6, which must. The v4 session beside it, same key and same code path bar the checksum, never missed a packet. Once again the log said nothing and the capture said everything.

The receive window was where the RFC turned out to be load-bearing in a way that only showed up under restart. Three rules were missing and together they were a session that never came back: no upper edge, so most of the number space was admitted; a linear comparison where the RFC asks for an unsigned 32-bit circular one; and, worst, the window was never forgotten. RFC 5880 clears bfd.AuthSeqKnown after twice the detection time without a packet for a reason it states outright — so the sequence can resynchronise if the remote system restarts — and bfdd seeds its transmit sequence randomly, so without that rule a peer restart was a coin toss on whether the new seed landed above the watermark. Two of eight authenticated sessions stuck for good while all fifty-six unauthenticated ones came back; adding the upper bound alone made it seven of eight, every time, because then no restart could land inside the window at all. The resync belongs in the sweep and it took two wrong attempts to see why: the program validates authentication whether or not it is answering for the session, so a rejected packet dies in the driver and userspace never sees it, which means a resync living in the engine cannot fire and pulling the kernel's stale window back on every poll undoes it even when it does.

The same class of mistake then appeared one layer up, and it is the most interesting failure in the milestone. The sweep returns early for a session under demand hold — correctly, because the peer was asked to stop transmitting and calling the session dead over silence we requested would be wrong — but it was returning before it reached the window ageing. Demand mode is precisely where that window most needs to age: the peer can restart inside a silence no detection timer will ever end, and it comes back with a fresh random sequence that the stale window rejects in the driver, so userspace never sees the peer at all and the session reports Up against something that is gone. Nothing recovers from that on its own. The capture is 115 Down packets from the peer across 100 seconds, none answered, with auth-bad on that session at 3074 against 2 received packets. It is the same defect as the bfdd fix filed as FRRouting/frr#23281, on the other side of the wire, and it has to be fixed in both places for the same reason the resync belongs in the sweep.

Key chains needed the data plane to be given more than a key. A chain hands over on a clock — the outgoing key stops being used to sign before it stops being believed, so a packet already in flight still verifies — and a receiver holding only the key it transmits under refuses exactly the packets that overlap exists to keep. Nothing was going to arrive to prompt a change either: bfdd re-reads the lifetimes on every packet it sends or receives, so its own path rolls over without being told, but an offloaded session is not on that path and there is no expiry hook in the key chain to fire instead. So the whole chain goes down with the period in which each key may be sent and the period in which each may still be accepted, and the data plane decides, because it is the side holding the packet. A meticulous keyed-SHA1 session at 50 ms was watched across 329 seconds and 14977 frames while both ends moved from key id 2 to key id 3 within the same frame pair; every frame in the capture is state Up and the largest gap between any two is 51.1 ms.

Three fixes went upstream — the same sequence-window defect in bfdd, a keychain with no usable key running the session unauthenticated instead of refusing it, and a your_disc == 0 test made against session state rather than against the packet's State field — and one finding went upstream as a report with no patch attached, deliberately: bfdd computes an HMAC where RFC 5880 section 6.7.4 specifies a plain SHA1 over the packet with the key embedded in the digest field. The two do not interoperate, and since bfdd is the control plane on one side of every session here, this engine follows bfdd rather than the RFC. Changing it would break every deployed FRR, which is a decision for the community rather than for a patch.

The closing measurement was six sessions of the live mesh watched across two restarts, one of the peer and one of the engine, captured on the hypervisor bridge so neither end is also the observer. Authentication does not appear in the recovery times at all: the fastest and slowest cells in each column are split across authenticated and unauthenticated sessions, with 64 of 64 up at both ends before and after, exactly one down event per watched session, and auth-bad at zero throughout. What does appear is demand mode, in both directions. When the peer restarts, the async sessions look instant only because the engine's own detect timeout had already fired and it was sitting there transmitting Down with your_disc zero, so the returning peer went straight to Init; the symmetric-demand session took 951 ms because nothing detected anything and recovery was paced by the restarted peer's one-second slow timer instead. When the engine restarts, the two demand cases finish in under a millisecond for the mirror-image reason — the peer's detection was held so it never went Down — while the async cases wait between 125 and 566 ms on the peer's slow-timer grid. And the demand blind spot has a security-shaped corollary worth stating plainly: changing the key on one end of the mesh took down seven of eight authenticated sessions on detect timeout exactly as intended, and left the eighth, demanding at both ends, Up throughout. Neither side transmits, so neither receives anything to reject. Authentication protects the packets a session sends; it cannot protect a session that has agreed to stop sending.
