...
`investigations/sweep-ladder/` established that in engine mode detection is entirely
...
(`investigations/sweep-ladder/`). Starved, detection runs about 31x the 30ms budget it
exists to enforce.
...
The root `README.md` and `benchmarks/README.md` result 2 both attributed
detection overshoot to 5ms sweep quantization and described detection as
kernel-side. `investigations/sweep-ladder/` falsified the first and this document the
...
than independently of it. `milestones/01-baseline/` carried the same quantization claim
