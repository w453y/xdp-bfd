...
void echo_tx_init(const char *ifname)
{
	...
	log_info("echo-tx: ifindex %d src-mac %02x:%02x:%02x:%02x:%02x:%02x sock %d\n",
		 echo_ifindex, echo_src_mac[0], echo_src_mac[1], echo_src_mac[2], echo_src_mac[3],
...
void echo_tx_maybe(struct session *s, uint64_t t)
{
	...
	if (sendto(echo_sock, frame, flen, 0, (struct sockaddr *)&sll, sizeof(sll)) < 0)
		return;
	...
	if (s->echo_tx_pkts % 100 == 0) {
		...
		log_debug("echo %s->%s tx=%llu rx=%llu lost=%llu rtt last/min/avg/max %llu/%llu/%llu/%llu us win-max %llu gap-max %lluus echo-alive=%d\n",
			  ...
			  (unsigned long long)s->echo_rtt_min_us, (unsigned long long)avg,
			  (unsigned long long)s->echo_rtt_max_us,
			  (unsigned long long)s->echo_rtt_max_win_us,
			  (unsigned long long)s->echo_gap_max_us, s->echo_alive_k);
		s->echo_gap_max_us = 0;
