...
rounded up and the loop ran no faster. `docs/tick-ladder/` measures it: at
...
`docs/tick-ladder/` was rerun afterwards and gave the same 0.09ms mean and
...
- *Quantization.* Detection notice was said to be quantized by the poll tick on
  top of the sweep. `docs/sweep-ladder/` showed the sweep contributes nothing in
...
about 950ms because `fsm_detect` cannot run (`docs/starved-detection/`). The
